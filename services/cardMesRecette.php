...
$cherchNbRecette = $db->prepare('SELECT COUNT(recetteId) AS nbrecette   FROM recette WHERE userId =:pId');
$cherchNbRecette ->execute ([

'pId' => $pseudoId,

]);

foreach($cherchNbRecette as $i )
{
...
$recette = $db->prepare('SELECT TIME_FORMAT(recetteDuree, "%H:%i") , recetteNom , pourCombien , difficulteNom , userPseudo , recetteId FROM recette INNER JOIN difficulte ON recette.difficulteId = difficulte.difficulteId INNER JOIN userT ON recette.userId = userT.userId WHERE recette.userId = :pId');
$recette ->execute ([
    ...
    <div id="blocCard"  >
        <div class="card">
          <div class="card-header">
            <img src="./img/fond3.png" alt="gateau" />
          </div>
          <div class="card-body">
            <span class="tag tag-purple">
    
            <?php echo "<a href='voirRecetteCo.php?test=".$idRecette."'>VOIR</a> " ?>
